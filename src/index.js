...
export async function run(options) {
  ...
  console.log(
    boxMessage(
      "An AI-powered tool for analyzing git diffs and posting comments to GitHub",
...
async function processRepository({
  ...
  if (commitsToProcess.length === 0) {
    console.log(
      boxMessage("All commits have already been analyzed", "Status", "success"),
    );
    return;
  }
  ...
  console.log(
    boxMessage(
      `Analyzed ${currentCommit} commits`,
      "Analysis Complete",
      "success",
    ),
  );
  ...
  if (options.dryRun) {
    console.log(
      boxMessage(
        "Dry run mode: Skipping posting comments to GitHub",
        "Dry Run",
        "warning",
      ),
    );
    ...
    for (const result of analysisResults) {
      table.push([
        ...
        result.commitMessage.substring(0, 27) +
          (result.commitMessage.length > 27 ? "..." : ""),
        result.summary.substring(0, 47) +
          (result.summary.length > 47 ? "..." : ""),
      ]);
  ...
  console.log(
    boxMessage(
      `Posted ${postedCount} comments to GitHub`,
      "Comments Posted",
      "success",
    ),
  );
  ...
  console.log(
    boxMessage("CommitStudio completed successfully!", "Complete", "success"),
  );
}
