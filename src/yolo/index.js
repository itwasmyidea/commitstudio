...
export async function runYolo(options) {
  ...
  console.log(boxMessage('YOLO MODE - Rewriting commit messages with AI', '🤪 CommitStudio YOLO 🤪', 'warning'));
 ...
 * @returns {Promise<void>}
 */
async function modifyCommitMessages({ options, owner, repo, branch, repoPath }) {
  ...
  spinner.succeed();
  console.log(boxMessage(`Found ${chalk.blue(commits.length)} commits to process`, 'Commits', 'info'));
  
  // Get OpenAI API key
  const apiKey = global.openaiApiKey || process.env.OPENAI_API_KEY;
  
  if (!apiKey) {
    console.error(boxMessage("OpenAI API key is required for YOLO mode", "Error", "error"));
    process.exit(1);
  }
  ...
  for (let i = 0; i < commits.length; i++) {
    ...
    try {
      ...
      console.error(`Error generating message for ${commit.sha.substring(0,7)}: ${error.message}`);
      results.push({
        sha: commit.sha,
        oldMessage: commit.message,
        newMessage: null,
